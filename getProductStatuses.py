...
class getProductStatuses:
    def __init__(self,apiKey = '',sign = '',time = '',productIdList = {'item' : []},itemIdList = {'item' : []},lang = '',session = None):
        ...
        with client.settings(raw_response=True):
            try:
                response = helpers.serialize_object(service.getProductStatuses(apiKey,sign,time,productIdList,itemIdList,lang).content.decode('utf-8'),dict)
                #Parsing...
                ...
                jsonList = jsonload['env:Envelope']['env:Body']['ns0:getProductStatusesResponse']['return']
