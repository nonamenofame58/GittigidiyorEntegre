...
class markAsRead:
    def __init__(self,apiKey = '',sign = '',time = '',markAsReadRequest = {'conversationId' : '','lang' : ''},session = None):
        ...
        with client.settings(raw_response=True):
            try:
                response = helpers.serialize_object(service.markAsRead(apiKey,sign,time,markAsReadRequest).content.decode('utf-8'),dict)
                #Parsing...
                ...
                jsonList = jsonload['env:Envelope']['env:Body']['ns0:markAsReadResponse']['return']
