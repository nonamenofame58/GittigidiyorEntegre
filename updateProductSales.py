...
class updateProductSales:
    def __init__(self,apiKey = '',sign = '',time = '',productUpdateSaleTypeArray  = {'productUpdateSales' : [{'item' : {'productId' : '' , 'price' : '','quantity' : '','updateSaleFields' : [{'field' : ''}, {'field' : ''}]}}]},lang = '',session = None):
        ...
        with client.settings(raw_response=True):
            try:
                response = helpers.serialize_object(service.updateProductSales(apiKey,sign,time,productIds,lang).content.decode('utf-8'),dict)
                #Parsing...
                ...
                jsonList = jsonload['env:Envelope']['env:Body']['ns0:updateProductSalesResponse']['return']
