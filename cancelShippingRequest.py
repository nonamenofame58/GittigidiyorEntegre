...
class cancelShippingRequest:
    def __init__(self,apiKey = '',sign = '',time = '',lang = '',saleCode = '',session = None):
        ...
        with client.settings(raw_response=True):
            try:
                response = helpers.serialize_object(service.cancelShippingRequest(apiKey,sign,time,lang,saleCode).content.decode('utf-8'),dict)
                #Parsing...
                ...
                jsonList = jsonload['env:Envelope']['env:Body']['ns0:cancelShippingRequestResponse']['return']
