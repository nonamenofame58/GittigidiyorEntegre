...
class putMessage:
    def __init__(self,apiKey = '',sign = '',time = '',putMessageRequest = {'conversationId' : '','message' : '','sendCopy' : '','lang' : ''},session = None):
        ...
        with client.settings(raw_response=True):
            try:
                response = helpers.serialize_object(service.putMessage(apiKey,sign,time,putMessageRequest).content.decode('utf-8'),dict)
                #Parsing...
                ...
                jsonList = jsonload['env:Envelope']['env:Body']['ns0:putMessageResponse']['return']
